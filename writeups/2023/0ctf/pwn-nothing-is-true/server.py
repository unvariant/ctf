...
def check_elf(data):
    ...
    print(hex(elf.header.e_ehsize))
    print(hex(elf.header.e_phoff))
    print(hex(elf.header.e_phnum))
...
def main():
    ...
    if check_elf(data):
        ...
        os.chmod(path, 0o555)
        try:
            p = subprocess.Popen(['docker', 'run', '-i', '--rm', '-v', f'{path}:/chroot/{filename}', 'launcher', filename], stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
            p.wait()
